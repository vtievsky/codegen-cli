...
func GenHTTPClient(
	...
	if err != nil {
		...
		return fmt.Errorf("failed to generate code for httpclient | %w", err)
	}
	...
	if respCli.HTTPResponse.StatusCode != http.StatusOK {
		...
		return fmt.Errorf("failed to generate code for httpclient | status: %s", respCli.HTTPResponse.Status)
	}
